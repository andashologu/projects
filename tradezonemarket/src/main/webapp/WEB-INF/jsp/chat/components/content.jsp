    ...
    <div class="field-wrapper-2 small-margin">
        ...
        <c:choose>
            <c:when test = "${isActive}">
                <label id="status_${chat.get().getId()}" class="label-field smaller-text light">Available</label>
            </c:when>
            <c:otherwise>
                <label id="status_${chat.get().getId()}" class="label-field smaller-text light">Away</label>
            </c:otherwise>
        </c:choose>
...
<div class="flexi-content-wrapper">
    <div class="flexi-content messages">
        ...
        <div style="padding: 10px;" id="messages" class="field-wrapper-2"></div><!--there must be no space inside this element in order firstchild to return null if empty-->
    </div>
</div>
